...
def delete_chet(path):  # удаление четных элементов в файле
    with open(path, "rb+") as file:
        ...
        for i in range(max_size):
            ...
            d = file.read(SIZE_)
            s = int(list(st.unpack("=l", d))[0])
            if s % 2 != 0 and l_ind != -1:
                file.seek(l_ind*SIZE_)
                file.write(d)
                l_ind += 1
            else:
                if s % 2 == 0:
                    if l_ind == -1:
                        l_ind = (file.tell() - SIZE_)//SIZE_
        file.truncate(max_pos - count*SIZE_)
